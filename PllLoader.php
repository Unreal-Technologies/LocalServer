...
class PllLoader
{
    public static function initialize(string $library, bool $debugmode = false)
    {
        ...
        if(file_exists($file))
        {
            PllLoader::fileMode($file, $version, $debugmode);
        }
     ...
     * @return void
     */
    private static function fileMode(string $file, string $version, bool $debugmode): void
    {
        $pi = pathinfo($file);
        ...
        spl_autoload_register(function(string $className) use($file, $pi, $version, $debugmode)
        {
            list($ns, $class) = self::parseClass($className);
            $nsFile = $pi['filename'].'/'.$version.'/'.preg_replace('/[^a-z0-9]+/i', '+', $ns).'.php';
            if(file_exists($nsFile) && !$debugmode)
            {
                require_once $nsFile;
                return;
            }
